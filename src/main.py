...
# Encodage des colonnes catégoriques (correspondance avec votre entraînement)
encoder_situationFamiliale = LabelEncoder()
encoder_situationFamiliale.fit(['En Couple', 'Célibataire', 'Marié(e)', 'Divorcée'])

encoder_2eme_voiture = LabelEncoder()
encoder_2eme_voiture.fit([False, True])
...
if st.button("Prédire la Catégorie"):
    ...
    input_data = pd.DataFrame({
        "age": [age],
        "taux": [taux],
        "situationFamiliale": [encoder_situationFamiliale.transform([situation_familiale])[0]],
        "nbEnfantsAcharge": [nb_enfants],
        "2eme voiture": [encoder_2eme_voiture.transform([deuxieme_voiture])[0]],
    })
